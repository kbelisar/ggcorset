...
## FOR LONG-FORM DATA
gg_corset_elongated <- function(data, x_var, x_vals, y_var, group, c_var, eyelets = FALSE, e_type = "SE", faceted = FALSE, vio_fill = NA, line_size = NA) {
    ...
    if(e_type=="SE") {
      ...
      data.summ <- data %>% group_by(c_var, x_var) %>%
        summarize(mean_y = mean(y_var, na.rm = T),
                  sd_se = sqrt(var(y_var,na.rm = T)/length(y_var)))
    }
